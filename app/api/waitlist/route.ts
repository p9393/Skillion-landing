...
// ─── In-memory IP rate limiter (per waitlist — public endpoint) ─────────────
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!apiKey) {
      // Dev mode: no emails
      if (isDev) console.log("[waitlist] Dev mode — skipping emails.");
      return NextResponse.json({ success: true });
    }
    ...
    if (!notifyRes.ok) {
      ...
      console.error("[waitlist] Admin notify error:", err);
    }
    ...
    if (!confirmRes.ok) {
      ...
      console.error("[waitlist] User confirmation error:", err2);
    }
